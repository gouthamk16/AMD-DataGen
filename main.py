...
dloader=dataloader(64)
gen=VAE_GAN().to(device)
dis=Discriminator().to(device)
real_batch = next(iter(dloader))
show_save("training" ,make_grid((real_batch[0]*0.5+0.5).cpu(),8))
...
condition=nn.BCELoss().to(device)
opt_E=torch.optim.RMSprop(gen.encoder.parameters(), lr=lr)
opt_D=torch.optim.RMSprop(gen.decoder.parameters(), lr=lr)
opt_Dis=torch.optim.RMSprop(discrim.parameters(), lr=lr*alpha)
fixed_z=Variable(torch.randn((64,128))).to(device)
fixed_x=Variable(real_batch[0]).to(device)
...
for epoch in range(epochs):
  ...
  for i, (data,_) in enumerate(data_loader, 0):
    ...
    output = dis(datav)[0]
    errD_real = condition(output, ones_label)
    dis_real_list.append(errD_real.item())
    output = dis(rec_enc)[0]
    errD_rec_enc = condition(output, zeros_label)
    dis_fake_list.append(errD_rec_enc.item())
    output = dis(x_p_tilda)[0]
    errD_rec_noise = condition(output, zeros_label1)
